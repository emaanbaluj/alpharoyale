...
export async function POST(request: Request) {
  const { orderId, playerId, triggerPrice, quantity } = await request.json();
  ...
  if (order.order_type !== 'TAKE_PROFIT' && order.order_type !== 'STOP_LOSS') {
    return NextResponse.json({ error: 'Only TP/SL orders can be updated via this endpoint' }, { status: 400 });
  }
  ...
  if (triggerPrice !== undefined && triggerPrice !== null) {
    updates.trigger_price = triggerPrice;
  }
