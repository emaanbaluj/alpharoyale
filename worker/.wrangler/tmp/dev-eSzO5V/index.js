...
async function fetchPriceDataFromDB(supabase, symbol, limit = 200) {
  const { data, error } = await supabase.from("price_data").select("*").eq("symbol", symbol).order("timestamp", { ascending: false }).limit(limit);
  if (error) throw new Error(`Supabase error: ${error.message}`);
  return data ?? [];
}
__name(fetchPriceDataFromDB, "fetchPriceDataFromDB");
async function fetchGamePlayersFromDB(supabase, gameId, userId) {
...
__name(fetchGamePlayersFromDB, "fetchGamePlayersFromDB");
async function updateGamePlayerBalanceInDB(supabase, gameId, userId, newBalance, newEquity) {
...
__name(updateGamePlayerBalanceInDB, "updateGamePlayerBalanceInDB");
async function fetchPositionsFromDB(supabase, gameId, playerId, status) {
...
__name(fetchPositionsFromDB, "fetchPositionsFromDB");
async function insertPositionInDB(supabase, gameId, playerId, symbol, side, quantity, entryPrice, leverage = 1) {
...
__name(insertPositionInDB, "insertPositionInDB");
async function fetchOrdersFromDB(supabase, gameId, status, orderType) {
...
__name(fetchOrdersFromDB, "fetchOrdersFromDB");
async function updateOrderInDB(supabase, orderId, status, filledPrice) {
  const updates = { updated_at: (/* @__PURE__ */ new Date()).toISOString() };
  if (status !== void 0) updates.status = status;
  if (filledPrice !== void 0) {
    updates.filled_price = filledPrice;
    updates.filled_at = (/* @__PURE__ */ new Date()).toISOString();
  }
  const { error } = await supabase.from("orders").update(updates).eq("id", orderId);
  if (error) throw new Error(`Supabase error: ${error.message}`);
}
__name(updateOrderInDB, "updateOrderInDB");
async function insertOrderExecutionInDB(supabase, orderId, gameId, playerId, symbol, side, quantity, executionPrice, gameState) {
...
__name(insertOrderExecutionInDB, "insertOrderExecutionInDB");

// src/game.ts
async function processMarketOrders(supabase, gameId, tick) {
  const marketOrders = await fetchOrdersFromDB(supabase, gameId, "pending", "MARKET");
  if (marketOrders.length === 0) return;
  const players = await fetchGamePlayersFromDB(supabase, gameId);
  ...
  for (const order of marketOrders) {
    if (order.side !== "BUY") {
      await updateOrderInDB(supabase, order.id, "rejected");
      continue;
    }
    ...
    await updateOrderInDB(supabase, order.id, "filled", fillPrice);
    await insertOrderExecutionInDB(
      supabase,
      order.id,
      gameId,
      order.player_id,
    ...
    await updateGamePlayerBalanceInDB(
      supabase,
      gameId,
      order.player_id,
      newBalance,
...
__name(processMarketOrders, "processMarketOrders");
async function getLatestPricesBySymbol(supabase, symbols) {
  const map = /* @__PURE__ */ new Map();
  for (const sym of symbols) {
    const rows = await fetchPriceDataFromDB(supabase, sym, 1);
    if (rows[0]) map.set(sym, Number(rows[0].price));
  }
  return map;
}
__name(getLatestPricesBySymbol, "getLatestPricesBySymbol");
...
var src_default = {
  async fetch(request, env) {
    ...
    const gameId = url.searchParams.get("game_id") ?? void 0;
    ...
    if (url.pathname === "/test/game/market") {
      ...
      const beforeOrders = await fetchOrdersFromDB(
        supabase,
        gameId,
        "pending",
        "MARKET"
      );
      const beforePositions = await fetchPositionsFromDB(
        supabase,
        gameId,
        void 0,
        "open"
      ...
      const afterOrders = await fetchOrdersFromDB(supabase, gameId);
      const afterPositions = await fetchPositionsFromDB(
        supabase,
        gameId,
        void 0,
        "open"
...
// .wrangler/tmp/bundle-yrvypc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yrvypc/middleware-loader.entry.ts
