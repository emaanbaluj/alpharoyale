...
async function fetchPriceDataFromDB(supabase, symbols, limit = 200) {
  const { data, error } = await supabase.from("price_data").select("id").in("symbol", symbols).order("timestamp", { ascending: false }).limit(limit);
  if (error) {
    const details = error.details;
    const hint = error.hint;
    const code = error.code;
    throw new Error(
      `Supabase error: ${error.message}` + (code ? ` (code: ${code})` : "") + (details ? ` | details: ${details}` : "") + (hint ? ` | hint: ${hint}` : "")
    );
  }
  return data ?? [];
}
...
var src_default = {
  async fetch(request, env) {
    ...
    const symbols = (url.searchParams.get("symbols") ?? "BTC").split(",").map((s) => s.trim().toUpperCase()).filter(Boolean);
    const limit = Math.min(Number(url.searchParams.get("limit") ?? "50"), 500);
    try {
      const rows = await fetchPriceDataFromDB(supabase, symbols, limit);
      return Response.json({ ok: true, symbols, count: rows.length, rows });
    } catch (e) {
...
// .wrangler/tmp/bundle-qlpXod/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qlpXod/middleware-loader.entry.ts
